...
pthread_mutex_t png_lock = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t frontier_lock = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t hashmap_lock = PTHREAD_MUTEX_INITIALIZER;
...
int find_http(char *fname, int size, int follow_relative_links, const char *base_url, CSTACK *p);
...
int process_data(CURL *curl_handle, RECV_BUF *p_recv_buf, CSTACK *frontier, CSTACK *png);
htmlDocPtr mem_getdoc(char *buf, int size, const char *url)
{
...
int find_http(char *buf, int size, int follow_relative_links, const char *base_url, CSTACK *frontier)
{
    //printf("Finding http\n");
    int i;
    htmlDocPtr doc;
...
CURL *easy_handle_init(RECV_BUF *ptr, const char *url)
{
    ...
    //printf("Pass easy handle init if statements \n");
    ...
    //printf("Finished easy handle init \n");
...
int process_html(CURL *curl_handle, RECV_BUF *p_recv_buf, CSTACK *frontier)
{
    //char fname[256];
...
int process_png(CURL *curl_handle, RECV_BUF *p_recv_buf, CSTACK *png)
{
...
int sendRequest(char* url, CSTACK *frontier, CSTACK *png, CURL *curl_handle) 
{
...
int run(CSTACK* frontier, CSTACK* visited, CSTACK* png, int t, int m);
